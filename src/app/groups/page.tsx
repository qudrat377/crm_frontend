...
export default function GroupsPage() {
  ...
  const [branchId, setBranchId] = useState(""); // Sahifadagi filtr uchun
  const [showModal, setShowModal] = useState(false);

  // 1. React Hook Form sozlamalari
  const { register, handleSubmit, reset, control, watch, formState: { errors } } = useForm<Partial<Group>>({
    defaultValues: {
      status: "upcoming",
      maxStudents: 20,
      startTime: "09:00",
  ...
  // 2. Modal ichidagi tanlangan filialni kuzatish (Watch)
  const selectedBranchInModal = watch("branchId");

  // 3. Ma'lumotlarni yuklash (Serverdan)
  ...
  // MUHIM: Modal ochiq bo'lsa modal ichidagi filialni, bo'lmasa sahifadagi filialni yuboramiz
  const { data: courses } = useCourses(showModal ? selectedBranchInModal : (branchId || undefined));
  
  const { data: teachers } = useTeachers({ limit: 100, isActive: true });

  const createMutation = useMutationWithToast(
    (groupData: Partial<Group>) => groupsApi.create(groupData).then((r) => r.data.data),
    { successMessage: "Guruh muvaffaqiyatli yaratildi", invalidateKeys: ["groups"] }
  );

  const onSubmit = async (formData: Partial<Group>) => {
    try {
  ...
  const columns = [
      ...
      render: (_: any, row: Group) => (
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-brand-50 rounded-xl flex items-center justify-center">
            <BookOpen className="w-4.5 h-4.5 text-brand-600" size={18} />
          </div>
          <div>
            <p className="font-medium text-surface-900">{row.name}</p>
            <p className="text-xs text-surface-500">{row.course?.name}</p>
          </div>
        </div>
      ),
    },
    {
      ...
      title: "O'qituvchi",
      render: (_: any, row: Group) => (
        <span className="text-sm text-surface-700">{row.teacher?.fullName ?? "—"}</span>
      ),
    },
    {
      key: "scheduleDays",
    ...
    },
    {
      key: "status",
      title: "Holat",
      render: (v: any) => <Badge status={v || "upcoming"} />,
    },
  ];
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-surface-500 mt-0.5">Jami: {data?.total ?? 0} ta guruh</p>
            </div>
          ...
          <Card padding="none">
            {isLoading ? (
              ...
              <EmptyState icon={BookOpen} title="Guruhlar topilmadi" />
            ) : (
              <>
                <Table>
                  <TableHeader>
        ...
        >
          <form className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Controller
                ...
                render={({ field }) => (
                  <SearchableSelect
                    label="Kurs"
                    placeholder={selectedBranchInModal ? "Kursni tanlang" : "Avval filialni tanlang"}
                    options={courses?.map(c => ({ value: c.id, label: c.name })) || []}
                    value={field.value || ""}
                    onChange={(e) => field.onChange(e.target.value)}
                    error={errors.courseId?.message}
                    disabled={!selectedBranchInModal}
                  />
                )}
              />
              <Controller
                name="teacherId"
                control={control}
                rules={{ required: "O'qituvchini tanlang" }}
                render={({ field }) => (
                  <SearchableSelect
                    label="O'qituvchi"
                    placeholder="O'qituvchini tanlang"
                    options={teachers?.data?.map((t: any) => ({ value: t.id, label: t.fullName })) || []}
                    value={field.value || ""}
                    onChange={(e) => field.onChange(e.target.value)}
            ...
            <div className="grid grid-cols-3 gap-4">
              <Select label="Kunlar" options={DAYS} {...register("scheduleDays")} />
              <Input label="Boshlanish vaqti" type="time" {...register("startTime")} />
              <Input label="Tugash vaqti" type="time" {...register("endTime")} />
            </div>

            <div className="grid grid-cols-3 gap-4">
              <Input label="Maksimal o'quvchi" type="number" {...register("maxStudents")} />
              <Select
                label="Holat"
                options={[
                  { value: "upcoming", label: "Kutilmoqda" },
                ...
                {...register("status")}
              />
              <Input label="Sana" type="date" {...register("startedAt")} />
            </div>
          </form>
        </Modal>
      </DashboardLayout>
